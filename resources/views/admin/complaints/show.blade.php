<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Against: <a
                        href="{{ route('admin.users.show', $complaint->reportedUser->id) }}">{{ $complaint->reportedUser?->name }}</a>
                </p>
            </div>
    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white border rounded-2xl p-6 shadow-sm">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-800 mt-1">
                            <a
                                href="{{ route('admin.glasses.show', $complaint->glasses->id) }}">{{ $complaint->conversation?->glasses?->title ?? '—' }}</a>
                        </p>
                    </div>
                </div>
                    ...
                    <div class="p-5 border-t bg-gray-50">
                        <form method="POST" action="{{ route('admin.complaints.reply', $complaint->id) }}"
                            ...
                            <textarea name="body" rows="2" required
                                class="w-full border rounded-xl p-3 text-sm focus:ring-2 focus:ring-gray-900"
                                placeholder="Write a message..."></textarea>

                            <button type="submit"
                                class="bg-gray-900 hover:bg-gray-900 text-white font-semibold px-5 rounded-xl">
                                Send
                            </button>
                        </form>
                    </div>
            ...
            {{-- Close Complaint --}}
            @if(!in_array($complaint->status, ['resolved', 'dismissed']))
                <form method="POST" action="{{ route('admin.complaints.close', $complaint->id) }}">
                    @csrf
                    <button type="submit"
                        class="px-5 py-2.5 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-semibold">
                        Mark as Resolved
                    </button>
                </form>
            @endif
